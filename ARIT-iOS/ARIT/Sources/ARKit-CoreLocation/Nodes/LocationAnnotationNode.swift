...
//
//  Modified by Bradley Klemick on 02/08/2020.
//
import Foundation
import SceneKit
import CoreLocation
...
open class LocationAnnotationNode: LocationNode {
    ...
    public init(location: CLLocation?, image: UIImage) {
        let plane = SCNPlane(width: image.size.width / 10, height: image.size.height / 10)
    ...
    public init(location: CLLocation?, layer: CALayer) {
        let plane = SCNPlane(width: layer.bounds.size.width / 10, height: layer.bounds.size.height / 10)
    ...
    override func updatePositionAndScale(setup: Bool = false, scenePosition: SCNVector3?,
        ...
        SCNTransaction.animationDuration = setup ? 0.0 : 0.2
